 ...
 */
const PORT = 3003;
const online = new Map(); // branchId -> client
...
function emitClientsList(target) {
    const list = Array.from(online.values());
    const payload = { clients: list, count: list.length };
    if (target)
        target.emit("clients:list", payload);
    else
        io.emit("clients:list", payload);
}
function routeNotify(evt) {
...
io.on("connection", (socket) => {
    ...
    socket.on("client:join", (data) => {
        ...
        socket._branchId = data.branchId;
